...
var createModem = function() {
    ...
    /**
     * status code: https://m2msupport.net/m2msupport/atcpas-phone-activity-status/
     * */
    modem.getStatus = function(callback) {
        this.execute('AT+CPAS', function(data) {
            const regex = /: (\d)/gm;
            const str = data
            let m;

            return callback(null, regex.exec(str)[1])
        }.bind(this));
    }
    ...
    modem.sms = function(message, callback) {
        ...
        var appendPdu = function(response, escape_char) { //Response to a AT+CMGS=X is '>'. Which means we should enter PDU. If aything else has been returned, there's an error.
            if(escape_char !== '>'){
              return callback(new Error(_.join(_.filter([response, escape_char]),' '))); //An error has happened.
            }

            var job = this.execute(pdus[i]+String.fromCharCode(26), function(response, escape_char) {
                if(escape_char.match(/error/i)){
                  return callback(new Error(_.join(_.filter([response, escape_char]),' ')));
                }
